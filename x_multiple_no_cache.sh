...
for i in $(seq $NBR_LOOPS) ; do
  ...
  # run the test
  docker run -it --rm --name $CONTAINER_NAME -v $PWD:/src $DOCKER_IMAGE /bin/bash -c "cd /src; ls -l; povray +I$DATASET_NAME +Oframe-$i-$FRAME_FILE_SUFFIX +K.0  -H1024 -W1024; exit 0"
